 ...
 *   SB_PROD_VERIFY=1 with SUPABASE_URL / SUPABASE_PUBLISHABLE_KEY / SUPABASE_SECRET_KEY in the environment
...
export async function target(): Promise<Target> {
  if (process.env.SB_PROD_VERIFY === '1') {
    ...
    if (!anonKey || !serviceKey) throw new Error('SUPABASE_PUBLISHABLE_KEY / SUPABASE_SECRET_KEY missing');
  ...
  const anonKey = vars.PUBLISHABLE_KEY ?? vars.ANON_KEY ?? '';
