...
for (let i = 2; i < bomb.length; ) { const m = bomb[i + 1]; if (m === 0xc0 || m === 0xc2) { bomb.writeUInt16BE(30000, i + 5); bomb.writeUInt16BE(30000, i + 7); break; } i += 2 + bomb.readUInt16BE(i + 2); }
const polyglot = Buffer.concat([derivative, Buffer.from('PK\x03\x04 trailing zip payload <html><script>alert(1)</script>')]);
const withExif = Buffer.concat([derivative.subarray(0, 2), Buffer.from([0xff, 0xe1, 0x00, 0x10]), Buffer.from('Exif\0\0GPSDATA!!'), derivative.subarray(2)]);
...
for (const [name, buf, tier] of cases) {
  ...
  console.log(`${name.padEnd(42)} sniff=${sniff.padEnd(24)} ${r.ok ? `ACCEPT ${r.width}x${r.height} stored=${r.bytes}B dropped=${r.droppedBytes}B` : `REJECT ${r.code}${r.dims ? ` ${r.dims.width}x${r.dims.height}` : ''}`} (${ms}ms)`);
}
const s = sanitizeJpeg(withExif);
console.log(`\nsanitised EXIF sample still contains 'GPSDATA'? ${s.buffer.includes('GPSDATA') ? 'YES (BUG)' : 'no'}; polyglot tail survives? ${sanitizeJpeg(polyglot).buffer.includes('PK') ? 'YES (BUG)' : 'no'}`);
